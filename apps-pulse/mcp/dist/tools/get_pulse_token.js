...
export const getPulseTokenTool = {
    ...
    description: "Generate a short-lived Connected Apps JWT token for embedding a Tableau Pulse metric. " +
...
export async function getPulseToken(args) {
    ...
    if (!clientId || !secretId || !secretValue || !tableauHost || !siteName) {
        throw new Error("Missing required environment variables: CONNECTED_APP_CLIENT_ID, " +
    ...
    if (metricUrlObj.host !== expectedHost) {
        throw new Error(`Metric URL host (${metricUrlObj.host}) does not match configured Tableau host (${expectedHost})`);
    }
    // Validate site in URL matches configured site
    if (!metricUrl.includes(`/site/${siteName}/`)) {
        throw new Error(`Metric URL does not contain expected site: ${siteName}`);
    }
...
//# sourceMappingURL=get_pulse_token.js.map