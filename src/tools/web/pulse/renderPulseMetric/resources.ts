...
/**
 * Register the Pulse widget resource with an McpServer instance.
 *
 * Called by WebMcpServer when the render-pulse-metric tool is registered so
 * OpenAI Apps SDK hosts can fetch the widget HTML via resources/read.
 */
export function registerPulseWidgetResource(mcpServer: McpServer): void {
  mcpServer.registerResource(
    'Tableau Pulse Metric Widget',
    PULSE_WIDGET_URI,
    {
      description: 'Interactive Tableau Pulse metric visualization widget for OpenAI Apps SDK',
      mimeType: 'text/html+skybridge',
    ...
    async () => ({
      contents: [readPulseWidgetResource(PULSE_WIDGET_URI)],
    }),
  );
}
