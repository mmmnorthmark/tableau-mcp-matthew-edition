...
export const getFollowedPulseMetricsTool = (server: WebMcpServer): WebTool<typeof paramsSchema> => {
  const getFollowedPulseMetricsTool = new WebTool({
    server,
...
`,
    ...
    callback: async ({ returnGroups }, extra): Promise<CallToolResult> => {
      return await getFollowedPulseMetricsTool.logAndExecute({
        ...
        callback: async () => {
          return await useRestApi({
            ...extra,
            jwtScopes: getFollowedPulseMetricsTool.requiredApiScopes,
            callback: async (restApi) => {
              if (returnGroups) {
                ...
                const groupedResult: CallToolResult = {
                  content: [
                    {
                      type: 'text' as const,
                ...
                const flatResult: CallToolResult = {
                  content: [
                    {
                      type: 'text' as const,
