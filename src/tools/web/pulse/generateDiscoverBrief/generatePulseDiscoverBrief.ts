...
const paramsSchema = {
  ...
  role: z
    .enum(['ROLE_UNDEFINED', 'ROLE_USER', 'ROLE_ASSISTANT'])
    .optional()
    .default('ROLE_USER')
    .describe('The role of the user asking the question (default: ROLE_USER)'),
};

export const getGeneratePulseDiscoverBriefTool = (
  server: WebMcpServer,
): WebTool<typeof paramsSchema> => {
  const generatePulseDiscoverBriefTool = new WebTool({
    server,
...
`,
    ...
    callback: async ({ question, metricIds, actionType, role }, extra): Promise<CallToolResult> => {
      return await generatePulseDiscoverBriefTool.logAndExecute({
        ...
        callback: async () => {
          return await useRestApi({
            ...extra,
            jwtScopes: generatePulseDiscoverBriefTool.requiredApiScopes,
