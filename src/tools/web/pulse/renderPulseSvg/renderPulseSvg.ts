...
async function renderVegaSpec(vegaSpec: any): Promise<{
  ...
  const bounds = (view.scenegraph() as any)?.root?.bounds as ScenegraphBounds | undefined;
...
export const getRenderPulseSvgTool = (server: WebMcpServer): WebTool<typeof paramsSchema> => {
  const renderPulseSvgTool = new WebTool({
    server,
      ...
      return await renderPulseSvgTool.logAndExecute({
        ...
        callback: async () => {
          return await useRestApi({
            ...extra,
            jwtScopes: renderPulseSvgTool.requiredApiScopes,
            callback: async (restApi) => {
              ...
              if (!basicSpecification) {
                throw new Error(`Definition ${defId} is missing basic_specification`);
              }
