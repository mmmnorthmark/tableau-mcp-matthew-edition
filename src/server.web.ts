...
import { registerPulseWidgetResource } from './tools/web/pulse/renderPulseMetric/resources.js';
...
export class WebMcpServer extends Server {
  constructor({
    ...
    clientInfo,
    mcpServerUrl,
  }: { mcpServer?: McpServer; clientInfo?: ClientInfo; mcpServerUrl?: string } = {}) {
    super({ mcpServer, clientInfo, serverName, serverVersion, mcpServerUrl });
  }
  ...
  registerTools = async (tableauAuthInfo?: TableauAuthInfo): Promise<void> => {
    ...
    for (const tool of await this._getToolsToRegister(tableauAuthInfo)) {
      ...
      if (tool.name === 'render-pulse-metric') {
        // Expose the OpenAI Apps SDK widget HTML for the render-pulse-metric tool.
        registerPulseWidgetResource(this.mcpServer);
      }
    }
